...
class BaseStringFormatter(object):
    ...
    def checkconsumed(self):
        if self.values_pos < len(self.values_w) and self.w_valuedict is None:
            raise oefmt(self.space.w_TypeError, 'not all arguments '
    ...
    def std_wp_int(self, r, prefix=''):
        ...
        if self.prec >= 0:
            if self.prec > 1000:
                raise oefmt(self.space.w_OverflowError, 'formatted integer '
    ...
    class StringFormatter(BaseStringFormatter):
        ...
        def unknown_fmtchar(self):
            ...
            if do_unicode:
                ...
                w_s = space.call_method(space.wrapbytes(c),
                                        "decode", w_defaultencoding,
                                        space.wrap('replace'))
                s = space.unicode_w(w_s)
            msg = (u"unsupported format character '%s' (0x%x) at index %d" %
