    ...
    class TemplateFormatter(object):
        ...
        def _get_argument(self, name):
            ...
            if use_numeric:
                if self.auto_numbering_state == ANS_MANUAL:
                    if empty:
                        raise oefmt(space.w_ValueError, "switching from "
                                    "manual to automatic numbering")
                elif not empty:
                    raise oefmt(space.w_ValueError,
            ...
            if index == -1:
                ...
                if self.is_unicode:
                    ...
                    try:
                        arg_key = kwarg.decode('utf-8')
                    except UnicodeDecodeError:
                        raise oefmt(space.w_KeyError, 'Invalid key')
        ...
        def _convert(self, w_obj, conversion):
            ...
            if conv == "r":
                ...
                raise oefmt(self.space.w_ValueError, "invalid conversion")
...
def make_formatting_class():
    class Formatter(BaseFormatter):
        ...
        def _unknown_presentation(self, tp):
            rstring.assert_ascii(self._type)
            rstring.assert_ascii(tp)
            raise oefmt(self.space.w_ValueError,
        ...
        def format_string(self, w_string):
            ...
            if self._alternate:
                raise oefmt(space.w_ValueError, "Alternate form not allowed "
                            "in string format specifier")
            if self._align == "=":
                raise oefmt(space.w_ValueError, "'=' alignment not allowed "
                            "in string format specifier")
            length = len(string)
        ...
        def _format_complex(self, w_complex):
            ...
            if self._align == "=":
                ...
                raise oefmt(space.w_ValueError, "'=' alignment flag is not "
                            "allowed in complex format specifier")
            if self._fill_char == "0":
                #zero padding is invalid
                raise oefmt(space.w_ValueError, "Zero padding is not allowed "
                            "in complex format specifier")
            if self._alternate:
                flags |= rfloat.DTSF_ALT
