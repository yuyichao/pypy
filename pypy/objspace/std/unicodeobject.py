        ...
        if y is not None:
            ...
            try:
                ...
                if not e.match(space, space.w_TypeError):
                    raise
                raise oefmt(space.w_TypeError, "first maketrans argument must "
                            "be a string if there is a second argument")
            if len(x) != ylen:
                raise oefmt(space.w_ValueError, "the first two maketrans "
            ...
            if not space.is_w(space.type(w_x), space.w_dict):
                raise oefmt(space.w_TypeError, "if you give only one argument "
            ...
            while True:
                ...
                if space.isinstance_w(w_key, space.w_unicode):
                    ...
                    if len(key) != 1:
                        raise oefmt(space.w_ValueError, "string keys in "
                                    "translate table must be of length 1")
                    ...
                    try:
                        ...
                        if not e.match(space, space.w_TypeError):
                            raise
                        raise oefmt(space.w_TypeError, "keys in translate "
                                    "table must be strings or integers")
                space.setitem(w_new, w_key, w_value)
        return w_new
